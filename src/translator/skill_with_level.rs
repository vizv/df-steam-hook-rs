...
pub fn translate_skill_with_level(string: &String) -> Option<String> {
  let skill_levels_matcher = matcher::word_matcher(matcher::DictType::Dictionary(&data::SKILL_LEVELS.adjectives));
  let skill_names_matcher = matcher::word_matcher(matcher::DictType::Dictionary(&data::SKILL_NAMES.nouns));
